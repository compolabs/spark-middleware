...
use tokio_tungstenite::{
    connect_async, tungstenite::{client::IntoClientRequest, protocol::Message}, MaybeTlsStream, WebSocketStream
};
use tracing::{error, info, warn};
use futures_util::{SinkExt, StreamExt};


use crate::{config::env::ev, indexer::envio_event_handler::parse_envio_event, storage::matching_orders::MatchingOrders};
...
pub async fn listen_for_envio_events(
    ...
    let url = Url::parse(&ws_url)
        .map_err(|e| Error::Other(format!("Invalid ENVIO_WS_URL: {}", e).into()))?;
                    ...
                    match serde_json::from_str::<Value>(&text) {
                        Ok(json_val) => {
                            ...
                            match msg_type {
                                ...
                                "data" => {
                                    if let Some(payload) = json_val.get("payload").and_then(|p| p.get("data")) {
                                        if let Some(open_orders) = payload.get("OpenOrderEvent").and_then(|v| v.as_array()) {
                                            for order_value in open_orders {
                                                if let Some(event) = parse_envio_event(order_value, "OpenOrderEvent") {
                                                    handle_envio_event(order_book.clone(), matching_orders.clone(), event).await;
                                                }
                                            }
                                        }
                                        if let Some(cancel_orders) = payload.get("CancelOrderEvent").and_then(|v| v.as_array()) {
                                            for order_value in cancel_orders {
                                                if let Some(event) = parse_envio_event(order_value, "CancelOrderEvent") {
                                                    handle_envio_event(order_book.clone(), matching_orders.clone(), event).await;
                                                }
                                            }
                                        }
                                        if let Some(trade_orders) = payload.get("TradeOrderEvent").and_then(|v| v.as_array()) {
                                            for order_value in trade_orders {
                                                if let Some(event) = parse_envio_event(order_value, "TradeOrderEvent") {
                                                    handle_envio_event(order_book.clone(), matching_orders.clone(), event).await;
                                                }
                                            }
                                        }
...
pub async fn subscribe_to_envio_events(
    ...
    ws_stream
        .send(Message::Text(format_cancel_order_subscription(market).into()))
        .await?;

    // Подписка на TradeOrderEvent
    ws_stream
        .send(Message::Text(format_trade_order_subscription(market).into()))
        .await?;
