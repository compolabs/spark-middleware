...
impl MatcherWebSocket {
    ...
    async fn start_batch_processing(
        ...
        let batch_processor = BatchProcessor::new(self.settings.clone(), self.metrics_handler.clone());
        batch_processor.process_batches(uuid, ws_stream, sender, aggregator, matcher_manager).await;
    }
